...
class UpdateRecurringInvoiceTemplateRequest extends FormRequest
{
    /**
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Once a template has generated at least one invoice, its next_run_at
        // schedule is anchored to the original starts_at date — changing it
        // afterwards would desynchronize past and future billing periods, so
        // any submitted change to it is silently ignored past that point.
        $template = RecurringInvoiceTemplate::find((int) $this->route('template'));

        if ($template !== null && $template->occurrences_generated > 0) {
            $this->merge(['starts_at' => $template->starts_at->toDateString()]);
        }
    }
