...
export default function ProposalCreatePage({
    ...
    clients,
    projects,
}: ProposalCreatePageProps) {
    const { currentTeam } = usePage().props;
    const [title, setTitle] = useState(`Proposal #${nextNumber}`);
    const [datePrepared, setDatePrepared] = useState(defaultDatePrepared);
                    ...
                    >
                        <div className="mb-[40px] w-full max-w-[800px] rounded-[12px] border border-bion-border bg-bion-surface p-[40px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] max-[1024px]:p-[20px]">
                            ...
                            <div className="mb-[8px] flex items-center justify-between">
                                <div className="text-[12px] font-semibold text-bion-text-muted uppercase [letter-spacing:0.05em]">
                                    Proposal Content
                                </div>
                                <button
                                    type="button"
                                    className="inline-flex items-center gap-[6px] text-[12.5px] font-semibold text-bion-accent hover:underline"
                            ...
                            {showAiPanel ? (
                                <div className="mb-[16px] rounded-[8px] border border-dashed border-bion-border bg-bion-bg p-[16px]">
                                    <textarea
...
ProposalCreatePage.layout = (props: { currentTeam?: { slug: string } | null }) => ({
    breadcrumbs: [
        {
            title: 'Dashboard',
        ...
        {
            title: 'New Proposal',
            href: props.currentTeam ? proposalCreate(props.currentTeam.slug) : '/',
        },
    ],
