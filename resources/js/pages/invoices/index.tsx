...
const TAB_BTN_ACTIVE =
    'mr-[24px] whitespace-nowrap border-b-2 border-bion-accent py-[10px] text-[13.5px] font-medium text-bion-text';

const RECURRING_BADGE =
    'ml-[8px] inline-flex items-center rounded-full bg-bion-accent-soft px-[7px] py-[1px] text-[10.5px] font-medium text-bion-accent';

type TabKey = 'oneTime' | 'recurring';
...
export default function InvoicesPage({
    ...
    const visitRecurringTemplate = (templateId: number): void => {
        if (!currentTeam) {
            return;
        }
    ...
    const visitCreateRecurring = (): void => {
        if (!currentTeam) {
            return;
        }

        router.visit(recurringCreate(currentTeam.slug));
    };
        ...
        <>
            ...
            <div className="flex min-h-0 flex-1 flex-col">
                <p className="mb-[14px] shrink-0 text-[13px] text-bion-text-muted">
                    Manage your billing, track payments, and send invoices to
                    clients.
                </p>

                <div className="mb-[16px] flex shrink-0 gap-0 overflow-x-auto border-b border-bion-border">
                    {(
                        [
                            ['oneTime', 'One-time'],
                            ['recurring', 'Recurring'],
                        ...
                        <button
                            ...
                            type="button"
                            className={
                                activeTab === tab ? TAB_BTN_ACTIVE : TAB_BTN
                            }
                                    ...
                                    {filteredInvoices.map((invoice) => (
                                        ...
                                        >
                                            <td
                                                className={cn(
                                                    TD_CLS,
                                                    'font-mono text-[13px] text-bion-text-muted',
                                                )}
                                            >
                                                {invoice.number}
                                                {invoice.isRecurring ? (
                                            ...
                                            <td
                                                className={cn(
                                                    TD_CLS,
                                                    'text-[13px] text-bion-text-muted max-[760px]:hidden',
                                                )}
                                            >
                                                {invoice.context}
                                            </td>
                                            ...
                                            <td
                                                className={cn(
                                                    TD_CLS,
                                                    'text-right font-mono text-[14px] font-semibold',
                                                )}
                                            >
                                                {invoice.amount}
                                            </td>
                ...
                <div
                    className={cn(
                        'min-h-0 flex-col',
                        activeTab === 'recurring' ? 'flex' : 'hidden',
                    )}
                >
                    <div className="mb-[16px] flex shrink-0 items-center justify-end">
                        <button
                            type="button"
                            className={BTN_PRIMARY}
                            onClick={visitCreateRecurring}
                        >
                            <svg className={ICON_SM_CLS}>
                                <use href="#i-plus" />
                            </svg>
                            ...
                            <table className="w-full border-collapse">
                                <thead className="sticky top-0 z-[5] bg-bion-surface">
                                    <tr>
                                        ...
                                        <th className={TH_CLS}>Cadence</th>
                                        <th className={TH_CLS}>Next Invoice</th>
                                        <th className={TH_CLS}>Status</th>
                                        <th
                                            className={cn(TH_CLS, 'text-right')}
                                        >
                                    ...
                                    {recurringTemplates.map((template) => (
                                        ...
                                        >
                                            ...
                                            <td
                                                className={cn(
                                                    TD_CLS,
                                                    'text-[13px] text-bion-text-muted',
                                                )}
                                            >
                                                {template.title}
                                            </td>
