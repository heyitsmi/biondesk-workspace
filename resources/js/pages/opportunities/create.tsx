...
import { QuickAddContactDialog } from '@/components/biondesk/quick-add-contact-dialog';
import { cn } from '@/lib/utils';
import { dashboard } from '@/routes';
import {
    create as opportunityCreate,
    index as opportunities,
    store as storeOpportunity,
} from '@/routes/opportunities';
import type {
    OpportunityCreatePageProps,
...
export default function OpportunityCreatePage({
    ...
    const { data, setData, post, processing, errors } =
        useForm<OpportunityFormValues>(defaults);
    const [isQuickAddOpen, setIsQuickAddOpen] = useState(false);

    const contactOptions = useMemo(() => {
        if (
            !quickAddedContact ||
            contacts.some((contact) => contact.id === quickAddedContact.id)
        ) {
            return contacts;
        }

        return [...contacts, quickAddedContact];
    }, [contacts, quickAddedContact]);

    useEffect(() => {
        if (!quickAddedContact) {
            return;
        }

        setData('contactId', quickAddedContact.id);
    }, [quickAddedContact, setData]);
                ...
                <div className="w-full max-w-[680px] pb-[80px]">
                    <div className="mb-[24px]">
                        ...
                        <p className="text-[14px] text-bion-text-muted">
                            Track a new potential deal or project in your
                            pipeline.
                        </p>
                    </div>
                                ...
                                <div className="mb-[20px] grid grid-cols-2 gap-[20px] max-[760px]:grid-cols-1">
                                    ...
                                    <div className="flex flex-col gap-[8px]">
                                        <div className="flex items-center justify-between gap-[12px]">
                                            ...
                                            {currentTeam ? (
                                                <button
                                                    type="button"
                                                    className="text-[12px] font-semibold text-bion-accent hover:underline"
                                                    onClick={() =>
                                    ...
                                    <div className="flex flex-col gap-[8px]">
                                        ...
                                        <div className="relative flex items-center">
                                            <span className="absolute left-[14px] text-bion-text-muted">
                                                $
                                            </span>
                                            <input
                                                type="number"
...
OpportunityCreatePage.layout = (props: {
    ...
    breadcrumbs: [
        ...
        },
        {
            ...
            href: props.currentTeam
                ? opportunities(props.currentTeam.slug)
                : '/',
        },
        {
            title: 'New Opportunity',
            href: props.currentTeam
                ? opportunityCreate(props.currentTeam.slug)
                : '/',
        },
    ],
    mainClassName:
